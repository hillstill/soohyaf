...
<form class="form-signin" ng-app="myLogin" ng-controller="myLoginCtrl" ng-submit="processForm()">
	...
	<input type="password"	id="inputPassword"	ng-model="pwd"		class="form-control" placeholder="输入密码" required>
...
myLogin.controller("myLoginCtrl", function($scope,$http) {
	...
	$scope.processForm = function() {
			$http({		method: 'JSONP',  
						url: api_host+'passport/login&loginname='+encodeURI($scope.usr)+'&passwd='+encodeURI($scope.pwd)+'&valid='+encodeURI($scope.validcode)+'&__VIEW__=jsonp&jsonp=JSON_CALLBACK'})
